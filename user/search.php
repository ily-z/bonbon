...
<head>
  ...
  <title>Hasil Pencarian | Bonbon Bakery and Cake</title>
...
<div class="container mt-4">
  <h2 class="text-center section-title">
    Hasil Pencarian: <?php echo isset($_GET['cari']) ? htmlspecialchars($_GET['cari']) : ''; ?> <i class="bi bi-search text-warning"></i>
  </h2>
  <div class="row">
    <div class="col-md-3">
      <div class="sidebar">
        ...
        while($item = mysqli_fetch_array($new)) {
          echo "<a href='detail-produk.php?id={$item['id_barang']}' class='new-product d-flex align-items-center text-white text-decoration-none mb-3'>
    ...
    <div class="col-md-9">
      <div class="row">
        ...
        $limit = 9;
        $offset = ($page - 1) * $limit;
        $sql1 = "SELECT * FROM barang WHERE nama_barang LIKE '%$cari%' LIMIT $offset, $limit";
      ...
      <nav class="mt-4">
        <ul class="pagination justify-content-center">
          <?php
          $total = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM barang WHERE nama_barang LIKE '%$cari%'"));
          ...
          if ($start > 1) {
            echo "<li class='page-item'><a class='page-link' href='search.php?page=".($start - 1)."&cari=" . urlencode($cari) . "'><<</a></li>";
          }

          for ($i = $start; $i <= $end; $i++) {
            $active = ($i == $page) ? 'active' : '';
            echo "<li class='page-item $active'><a class='page-link' href='search.php?page=$i&cari=" . urlencode($cari) . "'>$i</a></li>";
          }

          if ($end < $pages) {
            echo "<li class='page-item'><a class='page-link' href='search.php?page=".($end + 1)."&cari=" . urlencode($cari) . "'>>></a></li>";
          }
          ?>
        </ul>
      </nav>
